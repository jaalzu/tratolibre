...
export function FormSelect({ value, onChange, options, placeholder, invalid, disabled }: FormSelectProps) {
          ...
          {options.map(opt => (
            <button
              ...
              style={{
                ...
                background: value === opt.id ? 'var(--chakra-colors-brand-50)' : 'transparent',
                border: 'none', cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              }}
              onMouseEnter={e => { if (value !== opt.id) (e.currentTarget as HTMLButtonElement).style.background = 'var(--chakra-colors-neutral-50)' }}
