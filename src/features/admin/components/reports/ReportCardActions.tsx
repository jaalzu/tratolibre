...
export function ReportCardActions({ report }: ReportCardActionsProps) {
  const { loading, deleteItem, dismissReport, markAsReviewed } = useReportActions()
  ...
  return (
    <>
      ...
      <Flex align="center" gap={2} mb={4}>
        <Box w={6} h={6} borderRadius="full" bg="brand.subtle" display="flex" alignItems="center" justifyContent="center">
      ...
      <Flex gap={2} align="center" flexWrap="wrap" justify="flex-end">
        <Button size="sm" variant="secondary" py={1.5} px={3} asChild _hover={{ bg: 'neutral.100' }}>
          <NextLink href={targetHref} target={report.type !== 'conversation' ? '_blank' : undefined}>
        ...
        {report.status === 'pending' && (
          <>
            {report.type === 'item' && (
              <Button
                size="sm" py={1.5} px={3}
                bg="neutral.50" color="brand.dark"
                border="1px solid" borderColor="brand.dark"
                _hover={{ bg: 'brand.100' }}
            ...
            <Button
              size="sm" py={1.5} px={3}
              bg="neutral.50" color="red.600"
              border="1px solid" borderColor="red.600"
              _hover={{ bg: 'red.50' }}
