...
export function NotificationsPanel({ notifications, loading, onClose }: NotificationsPanelProps) {
  return (
    <>
      ...
      >
        <Text fontWeight="bold" fontSize="sm" color="neutral.900">Notificaciones</Text>
        <Box as="button" onClick={onClose} color="neutral.400" _hover={{ color: 'neutral.600' }}>
          <X size={16} />
        </Box>
      </Flex>
      ...
      <Box overflowY="auto" maxH="360px" flex={1} pb="env(safe-area-inset-bottom)">
        {loading ? (
          <Flex justify="center" py={8}>
            <Text fontSize="sm" color="neutral.400">Cargando...</Text>
          </Flex>
