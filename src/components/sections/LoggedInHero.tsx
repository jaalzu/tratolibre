...
export function LoggedInHero({ name }: LoggedInHeroProps) {
    const firstName = name ? name.split(' ')[0].charAt(0).toUpperCase() + name.split(' ')[0].slice(1).toLowerCase() : null
  ...
  return (
    <Box px={{ base: 3, md: 8 }} pt={3} pb={1}>
      ...
      >
        <Box>
          <Heading fontSize={{ base: "xl", md: "2xl" }} fontWeight="bold"  color="neutral.900" mb={1}>
                {firstName ? `Bienvenido, ${firstName}` : 'Bienvenido'}        </Heading>
        ...
        <Flex gap={5} flexWrap="wrap" justify="center">
            <Button asChild size="sm" px={{ base: 8, md: 12 }} py={{ base: 1.5, md: 2 }} fontSize={{ base: "sm", md: "md" }}>
