...
function HeroSlideContent({ slide, priority }: { slide: HeroSlide; priority?: boolean }) {
  return (
    <>
      {/* Mobile */}
      <Box display={{ base: 'block', md: 'none' }}>
        <Box position="relative" w="100%" h="55vw" maxH="400px">
          <Image src={slide.image} alt={slide.title} fill style={{ objectFit: 'cover' }} priority={priority} />
        </Box>
      ...
      <Flex display={{ base: 'none', md: 'flex' }} h="320px" bg={slide.bg}>
        <Flex flex={1} direction="column" justify="center" pl="12%" pr={8}>
          <Heading fontSize="xl" maxW={{ base: '100%', md: '80%' }} fontWeight="bold" color="neutral.900" mb={4}>
            {slide.title}
          </Heading>
